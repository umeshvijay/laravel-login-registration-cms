<?php

use Illuminate\Support\Facades\Route;
use App\Http\Controllers\UserAuthentication;
/*
|--------------------------------------------------------------------------
| Web Routes
|--------------------------------------------------------------------------
|
| Here is where you can register web routes for your application. These
| routes are loaded by the RouteServiceProvider within a group which
| contains the "web" middleware group. Now create something great!
|
*/

Route::get('/', function () {
    return view('welcome');
});


Route::get('login',[UserAuthentication::class, 'login']);
Route::get('register', [UserAuthentication::class, 'register']);
Route::post('create', [UserAuthentication::class, 'create'])->name('auth.create');
Route::post('check', [UserAuthentication::class, 'check'])->name('auth.check');
Route::get('profile', [UserAuthentication::class, 'profile']);
Route::get('logout', [UserAuthentication::class, 'logout']);
